...
function Page1({ setDisable }) {
    ...
    useEffect(() => {
        if (adsTitle && phone && age && height && weight && hair && eye && measurement && intro) {
            setDisable(false)
        } else {
            setDisable(true)
        }
    ...
    return (
        <div className='h-full w-full space-y-4'>
            ...
            {type === 'agency' && <div className="space-y-2">
                ...
                <input placeholder='ex: agency_name' type="text" className='w-80 h-11 outline-[#6418C3] outline-1 bg-[#F5F5F5] text-sm rounded-xl pl-5' />
            </div>}

            <div className="space-y-2">
                <p className='text-sm font-bold'>Profile Title*</p>
                <input placeholder='ex: Stacy' type="text" className='w-80 h-11 outline-[#6418C3] outline-1 bg-[#F5F5F5] text-sm rounded-xl pl-5' value={adsTitle} onChange={e => dispatch(setAdsTitle(e.target.value))} />
            </div>

            <div className="space-y-2">
                <p className='text-sm font-bold'>Phone Number*</p>
                <input type="text" className='w-80 h-11 outline-[#6418C3] outline-1 bg-[#F5F5F5] text-sm rounded-xl pl-5' value={phone.number} onChange={e => dispatch(setPhone({ type: 'number', value: e.target.value }))} />
            </div>
            ...
            <div className="flex gap-5 flex-wrap">
                ...
                <div className="space-y-2">
                    ...
                    <select type="text" className='w-16 h-11 outline-[#6418C3] outline-1 text-center bg-[#F5F5F5] text-sm rounded-xl' value={weight} onChange={e => dispatch(setWeight(Number(e.target.value)))}>
                        ...
                        {[...Array(41)].map((e, i) => <option value={i + 40}>{i + 40}</option>)}
                    </select>
                </div>
